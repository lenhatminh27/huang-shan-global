    ...
    return (
        <section className="bg-[#f4fcf5] py-12 md:py-16">
            <div className="mx-auto max-w-8xl px-5 md:px-10">
                <Reveal variant="up">
                    <h2 className="text-[32px] font-semibold text-primary md:text-[34px]">Quy trình triển khai</h2>
                </Reveal>

                <StaggerGroup className="mt-7 grid gap-4 md:grid-cols-2" staggerDelay={0.09}>
                    {service.process.map((step, index) => (
                        <MotionItem key={step.title} variant={index % 2 === 0 ? "drift" : "pop"}>
                            <article
                                ...
                                <h3 className="text-[22px] font-semibold leading-tight md:text-[24px]">
                                    {step.title}
                                </h3>
